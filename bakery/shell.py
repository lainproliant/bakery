...
class Shell:
    ...
    async def __call__(self, *args, stdout = asyncio.subprocess.PIPE, stderr = asyncio.subprocess.PIPE, name = None, log = None, **kwargs):
        ...
        try:
            ...
            env = os.environ.copy()
            env.update(self.env)
            proc = await asyncio.create_subprocess_exec(*cmd_line, stdout = stdout, stderr = stderr, env = env, **kwargs)
            readline_tasks = {
