        ...
        def decorator(f):
            ...
            async def wrapper(*args, **kwargs):
                ...
                if (check_files or output_files or temp_files) and verbose:
                    long_name = "%s %s" % (
                        recipe_name,
                        ",".join(check_files or output_files or temp_files),
                    )
                log = JobLog(self.log, name=long_name)

                if DEBUG:
                    print(
                        "Recipe %s invoked here..." % recipe_qualname, file=sys.stderr
                    )
                ...
                def outputs_up_to_date():
                    return (
                        ...
                        and outputs_exist()
                        and (
                            (check_mtimes() and output_mtimes())
                            and (max(check_mtimes()) <= max(output_mtimes()))
                            or not check_files
                        )
                    )
                ...
                def coalesce_default_outputs():
                    if (
                        len(targets) == 1
                        and len(target_params) == 1
                        and not is_iterable(target_params[0])
                    ):
                ...
                async def build_recipe():
                    ...
                    if not outputs_up_to_date():
                        raise BuildError(
                            'Recipe "%s %s" failed to create the prescribed output: %s'
                            % (
                                recipe_name,
                                ", ".join(check_files),
                                ", ".join(output_files),
